"""Kuickpay online payment orchestration — P7."""
...
class KuickpayService:
    ...
    def __init__(
        ...
        *,
        ...
        kuickpay_client: KuickpayClient | None = None,
    ) -> None:
        self._repo = payment_gateway_repository
        self._settings = get_settings()
        self._client = kuickpay_client or KuickpayClient()
        self._settlement = PaymentSettlementHelper(
            ...
            default_bank_key="kuickpayDefaultBankAccountId",
        )
    ...
    async def initiate_payment(
        ...
        *,
        ...
        merchant_ref = f"KP-{uuid.uuid4().hex[:16].upper()}"
        ...
        if checkout_url is None and self._settings.kuickpay_merchant_id:
            checkout_url = (
                f"https://kuickpay.example/pay?"
                f"merchant={self._settings.kuickpay_merchant_id}&ref={merchant_ref}"
            )
        return {
            "transaction": row,
            ...
            "mode": mode,
            "message": "Live Kuickpay checkout"
            if mode == "api"
            else "Stub checkout. Set KUICKPAY_API_URL for production.",
        }

    def verify_webhook_signature(self, *, body: bytes, signature: str | None) -> bool:
        secret = (self._settings.kuickpay_webhook_secret or "").strip()
        if not secret:
            return True
        if not signature:
            return False
        ...
        *,
        ...
        if not sales_receipt_id:
            sales_receipt_id = await self._settlement.create_receipt_with_allocation(
                ...
                document_label="Kuickpay settlement receipt",
                auto_fifo=auto_fifo,
