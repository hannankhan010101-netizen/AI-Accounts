/** New customer — catalog §5.1. Minimal master; full custom-fields land in Phase 4.1. */
"use client";
...
import { partiesApi, type CreateCustomerInput } from "@/lib/api/tenant";
...
export default function NewCustomerPage() {
  ...
  const { enabled: autoCode, nextCode } = useAutoCodePreview("customer", {
    onPreview: (code) => {
  ...
  const mutation = useMutation({
    mutationFn: (input: CreateCustomerInput) => partiesApi.createCustomer(input),
    onSuccess: () => {
      clearDraftOnSuccess();
      router.push("/sales/customers");
    },
    onError: (err) =>
      setSubmitError(err instanceof ApiError ? err.message : "Could not create customer"),
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <PageHeader
        title="Add customer"
        breadcrumb="Home / Sales / Customers / Add"
        description="Minimal master record. Custom fields, credit terms, customer-product pricing land in Phase 4.1."
        actions={
          <>
